...
##Get the column values which need to be plotted into variables##
dataToPlotAP <- as.numeric(dataReq$Global_active_power)
dataToPlotRP <- as.numeric(dataReq$Global_reactive_power)
dataToPlotV <-  as.numeric(dataReq$Voltage)
...
##Divide the display as 2 rows and 2 columns to fit all the 4 plots##
par(mfrow = c(2,2))


##Create plots which will be stored in the above png file##
plot(dataTime, dataToPlotAP, type="l", xlab="", ylab = "Global Active Power")

plot(dataTime, dataToPlotV, type="l", xlab="datetime", ylab = "Voltage")
...
plot(dataTime, dataToPlotRP, type="l", xlab="datetime", ylab = "Global_reactive_power")
